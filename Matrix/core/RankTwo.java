...
public class RankTwo {
    ...
    static int two(int[][] array) {
        if (array.length == array[0].length) return (array[0][0] * array[1][1]) - (array[0][1] * array[1][0]);
        if (array.length > array[0].length) return twoRow(array);
        else return twoColumn(array);
    }

    static int twoRow(int[][] array) {
        int rank, northWest, northEast, southWest, southEast, upperZero = 0, upperOne = 1, lowerZero = 0, lowerOne = 1;
        boolean last = false;
		while (true) {
            ...
            if (lowerOne == array.length - 1) {
				northWest = array[0][array.length];
				northEast = array[0][0];
				southWest = array[1][array.length];
				southEast = array[1][0];
                last = true;
			}
    ...
    static int twoColumn(int[][] array) {
        int rank, northWest, northEast, southWest, southEast, upperZero = 0, upperOne = 1, lowerZero = 0, lowerOne = 1;
        boolean last = false;
        while (true) {
            ...
            if (lowerOne == array[0].length - 1) {
                northWest = array[array[0].length][0];
                northEast = array[array[0].length][0];
                southWest = array[0][1];
                southEast = array[0][1];
                last = true;
            }
